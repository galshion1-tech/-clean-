function [Y_res, clean_data, info] = clean_baseline(cfg, Y, mf_data, s, mf, paths)
%CLEAN_BASELINE 传统 CLEAN 基线算法，在匹配滤波域抑制直达波/强多径。
%
% 用法:
%   [Y_res, clean_data, info] = clean_baseline(cfg, Y, mf_data, s, mf);
%   [Y_res, clean_data, info] = clean_baseline(cfg, Y, mf_data, s, mf, paths);
%
% 输入:
...
%   paths    可选，路径结构体数组。若提供，则只在干扰路径附近搜索。
%
% 输出:
%   Y_res       CLEAN 后的匹配滤波残差。
%   clean_data  CLEAN 结果结构体。
%   info        算法信息结构体。
%
% 物理模型:
%   匹配滤波输出中的单条路径近似为:
%       Y_p(tau) = alpha_p * a(theta_p) * g(tau - tau_p)
%
%   其中:
%       g(tau) = conv(s, mf)
%
%   本基线算法只使用名义导向矢量:
%       a_nom = steering_vector(theta, cfg)
%
%   因此当真实数据由带相位误差的 a_true 生成时，传统 CLEAN 会存在
%   replica 失配残差。这正是后续校准感知 CLEAN 要对比的问题。
...
%% 2. 构造匹配滤波点扩散函数 g(tau)
% 若 X 中存在 alpha*a*s(t-tau0)，经过 matched_filter_bank 后，对应为:
%   alpha*a*g(tau-tau0)
%
% 这里 g = conv(s, mf, 'full')，峰值应位于 tau = 0 附近。
...
%% 4. 预计算名义导向矢量角度字典
% A_grid: M x K_angle。注意这里不使用 phi_true。
A_grid = steering_vector(angle_grid, cfg);
...
if initial_mask_energy <= eps
    warning('初始搜索区域能量接近 0，CLEAN 不执行迭代。');
    Y_res = R;
    ...
    info = make_info(cfg, t_mf, pc_delay_axis, pc_kernel, pc_peak_value, ...
        0, initial_mask_energy, initial_total_energy, 'zero_initial_search_energy');
...
for it = 1:max_iter
    ...
    % alpha_hat(theta, tau) = a^H(theta)y(tau) / (||a||^2 g(0))
    % 使用 bsxfun 兼容旧版 MATLAB，避免依赖隐式扩展。
    ...
    a_hat = A_grid(:, best_angle_idx);
    alpha_sub = loop_gain * alpha_hat;

    Y_replica = a_hat * (alpha_sub * g_shifted.');
...
%% 9. 信息结构体
info = make_info(cfg, t_mf, pc_delay_axis, pc_kernel, pc_peak_value, ...
    num_iter_done, initial_mask_energy, initial_total_energy, stop_reason);
...
function info = make_info(cfg, t_mf, pc_delay_axis, pc_kernel, pc_peak_value, ...
    num_iter_done, initial_mask_energy, initial_total_energy, stop_reason)
%MAKE_INFO 构造 CLEAN 信息结构体。

info = struct();

info.model = 'baseline_clean_in_matched_filter_domain';
info.algorithm = 'traditional_CLEAN_with_nominal_steering_vector';
info.uses_phase_calibration = false;

info.physical_model = ...
    'Y_p(tau)=alpha_p*a_nom(theta_p)*g(tau-tau_p)';
info.important_note = ...
    'baseline CLEAN does not use phase_error; mismatch residual is expected when data are generated by a_true';
